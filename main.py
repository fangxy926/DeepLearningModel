...
from data_loader import MNISTLoader, AlexNetMNISTLoader
import os
...
def train_AlexNet():
    ...
    data_loader = AlexNetMNISTLoader()
    model = AlexNet(output_class_units=10)
    # model.build_graph(input_shape=(None, 32, 32, 1))
    # model.summary()
    optimizer = tf.keras.optimizers.Adam(learning_rate=LR)
    ...
    for epoch in range(EPOCHS):
        for batch in range(num_train_batches):
            X, y = data_loader.get_batch_train(BATCH_SIZE)
            with tf.GradientTape() as tape:
                y_pred = model(X)
    ...
    print("test accuracy: %f" % sparse_categorical_accuracy.result())
    model.save_weights(filepath=model_save_dir + "alexnet_mnist.h5")
